		...
		char *search_by, char *search_action)
{
	...
	while (curr_book)
	{
		if (!strcmp(search_by, "Author") ||
				!strcmp(search_by, "author"))
			property = curr_book->author;
		if (!strcmp(search_by, "Title") ||
				!strcmp(search_by, "title"))
			property = curr_book->title;
		if (!strcmp(search_by, "Genre") ||
				!strcmp(search_by, "genre"))
	...
	if (match > 0)
	{
		if (match < 2)
		{
			...
			printf("FOUND %d BOOK _____________\n", match);
		}
		else
		{
			printf("______________ ");
			printf("FOUND %d BOOKS _____________\n", match);
		}
	}
...
void handle_book_search(book *library, char *search_action)
{
	...
	printf("\n>>>>>>>>>> Enter Book Property <<<<<<<<<<\n");
	printf("\n--> Example: use Author or author");
	printf("to search by author's name\n\n");
	...
	{
		if (search_book(library, search_str,
					search_by, search_action) == 0)
		{
